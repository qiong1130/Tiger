...
public class ClassTable {
	...
	// Duplication is not allowed
	public void put(String c, String id, ast.Ast.Type.T type) {
	...
	public ast.Ast.Type.T get(String className, String xid) {
		ClassBinding cb = this.table.get(className);
		ast.Ast.Type.T type = cb.fields.get(xid);
	...
	public void dump() {
		for (Iterator<String> cls_table = this.table.keySet().iterator(); cls_table
			...
			for (Iterator<?> fields = cls_bind.fields.keySet().iterator(); fields
				...
				ast.Ast.Type.T type = cls_bind.fields.get(key);
